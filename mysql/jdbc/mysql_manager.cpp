...
std::shared_ptr<MysqlManager> MysqlManager::getInstance() {
    static auto mysql_manager = std::make_shared<MysqlManager>();
...
int MysqlManager::registerUser(const std::string &name,
    ...
    try {
        ...
        auto stmt = std::make_unique<sql::PreparedStatement>(
            connect->connect_->prepareStatement("CALL reg_user(?,?,?,@result)"));
        stmt->setString(1, name);
        stmt->setString(2, email);
        stmt->setString(3, pass);
        stmt->execute();
        auto stmt_result = std::make_unique<sql::Statement>(
            connect->connect_->createStatement());
        auto result = std::make_unique<sql::ResultSet>(
...
int MysqlManager::registerTransaction(const std::string &name,
                                      const std::string &email,
                                      const std::string &pass,
...
bool MysqlManager::updatePass(const std::string &name,
                              const std::string &new_pass) {

}

bool MysqlManager::checkPass(const std::string &name,
                             const std::string &pass,
                             UserInfo &user_info) {

}

bool MysqlManager::testProcedure(const std::string &email,
                                 int &uid,
                                 std::string &name) {
