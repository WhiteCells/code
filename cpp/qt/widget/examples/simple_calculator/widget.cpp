...
Widget::Widget(QWidget *parent) :
    ...
    // 将按钮加入按钮组
    // findChildren模板函数，查找子类，返回list容器
...
void Widget::initUI() {
    ...
    QString global = "*{"
                     ...
                     "background-color: rgb(0,0,0);" // 设置背景色
...
QString Widget::formatDouble(double value, int decimalPlaces, bool roundUp) { // 保留小数和四舍五入功能
...
void Widget::showHistory(int choose) {
    ...
    if (choose == 1) {
        ...
        ui->lineEdit_history->setText(SpecialOperatorHistory[0].toString() + vec_oper[0].toString() + SpecialOperatorHistory[1].toString() + vec_oper[1].toString());
    }
}
