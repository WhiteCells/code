...
# 删除部门
# bug 父部门删除后子部门没有从数据库中删除
# 查询部门表中父部门 (为待删除部门的 id) 的部门（递归）
@bp.delete('/remove/<int:_id>')
@authorize("system:dept:remove", log=True)
def remove(_id):
    delete_dept = Dept.query.get(_id)

    if not delete_dept:
        return fail_api(msg='部门不存在')

    def dfs_delete_child_dept(parent_id):
        child_depts = Dept.query.filter_by(parent_id=parent_id).all()
        for dept in child_depts:
            db.session.delete(dept)
            db.session.commit()
            dfs_delete_child_dept(dept.id)

    dfs_delete_child_dept(_id)
    db.session.delete(delete_dept)
    db.session.commit()
    return success_api(msg='部门删除成功')
