...
void HoverFillButton::paintEvent(QPaintEvent *event) {
    ...
    switch (animation_type) {
        ...
        case AnimationType::DiagonalRectangle:                                                  // 斜着的矩形填充
            painter.save();                                                                     // 保存状态
            painter.translate(width() / 2.0, height() / 2.0);                                   // 将坐标原点移动到按钮中心
            painter.rotate(45);                                                                 // 将坐标系旋转45度
            painter.drawRect(QRectF(-paint_value / 2.0, -width() / 2.0, paint_value, width())); // 绘制矩形
            painter.restore();                                                                  // 恢复状态
            break;
        case AnimationType::BottomCircle: // 底部圆形填充
            painter.save();
            painter.translate(width() / 2.0, int(height() * 1.5));
            painter.drawEllipse(QPoint(0, 0), paint_value, paint_value);
            painter.restore();
            break;
